...
- **🤖 Auto — smart model routing** (button next to the model field,
  ...
  **Live rankings, not just a hardcoded list.** By default each profile
  ...
  by.
- **Relaunching picks up exactly where you left off — per project.** Not
  just chat history — the app remembers which project folder and which
  specific chat (including branches) were active in it, and restores both
  automatically. This is tracked **per project**, not just once globally:
  jump between two different projects and each one independently resumes
  its own last-active chat, not "default" every time. Branch three chats
  deep into a debugging session, switch to another project, come back
  later or relaunch entirely — you land exactly back on that branch, not
  the top-level chat. (The pointer for this lives inside each project's
  own `.codex-agent` folder, so it travels with the project itself; if a
  saved folder's been moved or deleted, it just falls back to normal
  behavior — nothing breaks.)
- **Upload files** — "⬆ upload file…" in the sidebar's Projects section
  opens your system file picker; whatever you select gets copied straight
  into the current project folder (supports multiple files at once), with a
  confirmation showing exactly where it landed. Mention the filename in
  your next message and the agent can read it like any other file in the
  project.
- **Persistent chats**: everything auto-saves per project folder, same as
  ...
  rename it.
- **Archive chats** you don't want cluttering the list but aren't ready to
  delete — the 🗄 icon on any chat tucks it away without losing it. Find
  archived chats again via "🗄 archived chats" at the bottom of the Chats
  section, where you can restore (↩) or permanently delete (✕) each one.
