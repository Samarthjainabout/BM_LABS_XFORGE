...
async function runAgentLoop(turnStartIndex) {
  ...
  try {
    ...
    broadcast({ type: 'idle', usage: lastUsage, contextLimit: contextLimitFor(lastUsedModel || model) });
  }
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/api/state' && req.method === 'GET') {
    return send(res, 200, {
      ...
      usage: lastUsage, contextLimit: contextLimitFor(lastUsedModel || model), lastUsedModel
    });
  }
