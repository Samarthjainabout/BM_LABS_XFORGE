...
- **Stop button** replaces Send while the agent is working — cancels the
  in-flight request immediately (a real abort, not just "stop after this
  step") and leaves everything up to that point exactly as it was, so you
  can edit your last message or just send a new one.
...
- **Provider/model switcher** in the top bar — change providers without
  ...
  across every major lab. The field is always free-typeable too, so you're
  never limited to what's in the dropdown.
